...
import { getPool } from "../../shared/db/postgresPool";
...
async function downloadImages(
  ...
  try {
    ...
    const pool = getPool();
    const result = await pool.query<DrinkImage>(
      "SELECT id, thumbnail FROM drinks WHERE thumbnail IS NOT NULL"
    );
    ...
    for (const drink of result.rows) {
      try {
        ...
        await pool.query(
          "UPDATE drinks SET thumbnail = $1 WHERE id = $2",
          [newUrl, drink.id]
        );
