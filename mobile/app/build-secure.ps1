...
$SecretName = "APIM-SUBSCRIPTION-KEY"
...
# Step 2: Retrieve APIM Subscription Key from Key Vault
Write-Host "[2/4] Retrieving APIM Subscription Key from Key Vault..." -ForegroundColor Yellow
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Host "Error details: $apimKey" -ForegroundColor Red
        exit 1
    }
    ...
    Write-Host "  ✓ Successfully retrieved APIM Subscription Key" -ForegroundColor Green
} catch {
...
$buildCommand = "flutter build apk --release --dart-define=`"APIM_SUBSCRIPTION_KEY=$apimKey`""
...
Write-Host ""
Write-Host "Security Note: APIM subscription key was retrieved securely from Azure Key Vault" -ForegroundColor Cyan
