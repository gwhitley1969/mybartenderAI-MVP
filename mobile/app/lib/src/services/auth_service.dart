...
class AuthService {
  ...
  Future<User?> signIn() async {
    try {
      ...
      // Note: state is automatically generated by flutter_appauth, don't add it manually
      final nonce = DateTime.now().millisecondsSinceEpoch.toString();
      final additionalParams = {
        ...AuthConfig.additionalParameters,
        'nonce': nonce,
        // Don't override response_type - let flutter_appauth handle it
      };
      ...
      // Step 1: Authorization request only (don't exchange code automatically)
      ...
      final AuthorizationResponse? authResponse = await _appAuth.authorize(authRequest);

      if (authResponse == null) {
        developer.log('Authorization cancelled by user', name: 'AuthService');
        return null;
      }

      developer.log('Authorization successful, got code: ${authResponse.authorizationCode != null}', name: 'AuthService');

      // Step 3: Exchange authorization code for tokens (this will make a POST request)
      ...
      developer.log('Exchanging authorization code for tokens...', name: 'AuthService');
      ...
      developer.log('Token exchange successful, saving tokens', name: 'AuthService');
      ...
      // Create AuthorizationTokenResponse from the separate responses
      final combinedResult = AuthorizationTokenResponse(
      ...
      return await _handleAuthResult(combinedResult);
    } catch (e, stackTrace) {
      developer.log(
