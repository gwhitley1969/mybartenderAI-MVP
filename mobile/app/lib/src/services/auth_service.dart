...
class AuthService {
  ...
  Future<User?> signIn() async {
    try {
      ...
      // Interactive sign in
      final AuthenticationResult? result = await _msalAuth!.acquireToken(
        scopes: scopes,
  ...
  Future<void> signOut() async {
    try {
      ...
      if (_msalAuth == null) {
        developer.log('MSAL not initialized, clearing local tokens only', name: 'AuthService');
      ...
      await _tokenStorage.clearAll(); // Ensure local tokens are cleared even if sign out fails
      rethrow;
    }
  }
